...
// Copyright (c) 2013 C-Note developers
...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        // Parameters
        //
        // If Qt is used, parameters/c-note.conf are parsed in qt/bitcoin.cpp's main()
        ...
        {
            // First part of help message is specific to c-noted / RPC client
            std::string strUsage = _("C-Note version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  c-noted [options]                     " + "\n" +
                  "  c-noted [options] <command> [params]  " + _("Send command to -server or c-noted") + "\n" +
...
int main(int argc, char* argv[])
{
    ...
    // Connect c-noted signal handlers
    noui_connect();
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: c-note.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: c-noted.pid)") + "\n" +
        ...
        "  -port=<port>           " + _("Listen for connections on <port> (default: 18461)") + "\n" +
        ...
        "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 18460)") + "\n" +
        ...
        "\n" + _("SSL options: (see the C-Note Wiki for SSL setup instructions)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Make sure only a single C-Note process is using the data directory.
    boost::filesystem::path pathLockFile = GetDataDir() / ".lock";
    FILE* file = fopen(pathLockFile.string().c_str(), "a"); // empty lock file; created if it doesn't exist.
    if (file) fclose(file);
    static boost::interprocess::file_lock lock(pathLockFile.string().c_str());
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. C-Note is probably already running."), strDataDir.c_str()));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    printf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    printf("C-Note version %s (%s)\n", FormatFullVersion().c_str(), CLIENT_DATE.c_str());
